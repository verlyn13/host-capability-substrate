...
[`shell-environment-research.md`](./shell-environment-research.md) v2.12.0.
...
Current Claude app/settings overlay as of 2026-05-01:

| Surface | Evidence |
|---|---|
| Claude Desktop app bundle | `/Applications/Claude.app` metadata reports bundle id `com.anthropic.claudefordesktop`, version `1.5354.0`, build `1.5354.0` |
| Claude Desktop MCP config | Operator-provided settings identify `/Users/verlyn13/Library/Application Support/Claude/claude_desktop_config.json`; metadata-only local check observed mode `644`, size `2251`, modified `Apr 30 21:14:22 2026` |
| Filesystem tool permissions | Operator-provided app settings list read-only and write/delete filesystem tools as `ask`; "Copy file to Claude" is a separate copy/import surface |
| Claude Code Desktop settings | Operator-provided settings expose bypass mode, auto permissions mode, notifications, `.claude/worktrees`, branch prefix `claude`, Preview, persisted Preview sessions, and web PR/autofix/archive automation |
...
- Do not enable Claude bypass mode, auto permissions mode, Preview persistence
  inspection, or web PR/autofix automation as part of a test without a separate
  operation proof.
- Do not treat Claude app `ask` prompts as HCS `ApprovalGrant` records.
- Do not inspect persisted Preview cookies, local storage, login sessions, or
  raw Claude MCP config values unless a redacted operation plan explicitly
  authorizes the inspection.
...
| Claude app/settings | Claude Desktop and Claude Code Desktop settings ingest committed; runtime behavior remains surface-specific. | `research/shell-env/2026-05-01-claude-desktop-code-settings-ingest.md` |
...
- Codex app settings and Workspace Dependencies are app-managed surfaces. Do not
  ...
  probe.
- Claude app permission modes, Preview persistence, and web PR automation are
  app/cloud surfaces. Treat them as source intake only until a direct,
...
| Version | Date | Change |
|---|---|---|
| 1.10.0 | 2026-05-01 | Added Claude Desktop and Claude Code Desktop settings ingest status, guardrails, and app metadata overlay. |
