...
- **Charter v1.3.0 candidate invariant 17** — "execution-context is declared, not inferred": every operation carries a resolved `ExecutionContext.surface` reference; agents must not assume a subprocess inherits credentials, env, or sandbox scope from the parent context unless intentional inheritance is explicitly represented by a typed operator such as ADR 0016's Codex `inherit` vocabulary and bound to that execution context. Motivated by shell research v2 conclusions 1, 8, 9, 10; §VI.
