...
- **ADR 0020 proposed draft** — `docs/host-capability-substrate/adr/0020-version-control-authority.md` records the proposed Q-006 direction: version control is a typed authority surface, Git/GitHub facts start as evidence subtypes/receipts, check consumption requires expected-source identity, branch cleanup requires `BranchDeletionProof`, Actions posture is separate evidence, and source-control continuity is freshness-bound. Its receipt list is now split into five load-bearing Q-006 review names (`GitConfigResolution`, `GitIdentityBinding`, `BranchDeletionProof`, `StatusCheckSourceObservation`, `SourceControlContinuityReceipt`) plus a broader deferred inventory. ADR 0020 is proposed only; it accepts no GitHub mutation and does not displace Q-005/Q-008/Q-009.
- **Ontology promotion and receipt dedupe planning** — `docs/host-capability-substrate/research/local/2026-05-01-ontology-promotion-receipt-dedupe-plan.md` records the cross-Q review rule before additional Ring 0 schema work: observed facts start as evidence subtypes, durable lifecycle objects become standalone entities, and proof composites / authored coordination facts need their own sub-rules. Q-011 tracks human/reviewer approval of the promotion buckets, naming convention, duplicate dispositions, and dependency order. No schema or policy changes are accepted from this planning doc.
- **Version-control posture dashboard planning** — `docs/host-capability-substrate/dashboard-contracts.md` v0.2.1 adds a candidate read-only `VersionControlPosture` view model, `/source-control` route sketch, and per-surface capability state vocabulary (`proven`, `denied`, `pending`, `stale`, `inapplicable`). It is planning only: no dashboard route, schema, API endpoint, policy tier, GitHub setting, or mutation lane exists yet. The view exists to keep Q-006 evidence dashboard-renderable before any source-control mutation operation is designed.
...
- **ADR 0022 proposed draft** — `docs/host-capability-substrate/adr/0022-boundary-observation-envelope.md` records the proposed Q-007a direction: model `BoundaryObservation` as a freshness-bound `Evidence` subtype envelope first, keep `QualityGate` deferred, and use the envelope to reconcile macOS/TCC/app-bundle, package-manager, runner, source-control, execution-mode, and remote-agent boundary claims without making adapters or dashboards own policy. ADR 0022 is proposed only; it accepts no schema, generated JSON Schema, policy tier, dashboard route, or runtime probe.
...
- **Charter v1.3.0 candidate invariant 16** — "external-control-plane evidence-first": operations against remote control planes must produce typed evidence before provider-side mutations, distinguish provider object references / public client IDs / policy selector values / secret references / secret material, model separable provider validator surfaces before mutations that depend on them, and treat rate-limit/backoff state as evidence rather than retry pressure. Queue-only; v1.2.0 remains active until a deliberate Phase 1 charter amendment.
- **Charter v1.3.0 candidate invariant 17** — "execution-context is declared, not inferred": every operation carries a resolved `ExecutionContext.surface` reference; agents must not assume a subprocess inherits any sandbox, capability, environment, or credential scope from the parent context unless intentional inheritance is explicitly represented by typed evidence or a typed operator such as ADR 0016's Codex `inherit` / `include_only` vocabulary and bound to that execution context. Motivated by shell research v2 conclusions 1, 8, 9, 10; §VI.
- **ADR 0021 proposed draft** — `docs/host-capability-substrate/adr/0021-charter-v1-3-wave-1.md` packages charter v1.3.0 wave 1 for review: invariant 16 and invariant 17 only, voted independently, with invariants 18-20 deferred to Q-003/Q-007/Q-008. ADR 0021 is proposed only; it does not edit the active charter, which remains v1.2.0 until a separate charter amendment PR lands with required reviewer objections and human approval.
