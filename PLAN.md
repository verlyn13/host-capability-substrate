...
- **Ring-0 entity additions from shell research v2 (ADRs 0016/0017)**: `ExecutionContext` (per-surface type with sub-classes `codex_cli`, `codex_app_sandboxed`, `codex_ide_ext`, `claude_code_cli`, `claude_desktop`, `claude_code_ide_ext`, `zed_external_agent`, `warp_terminal`; each with shell+invocation+startup-files+sandbox+env-inheritance facets per §II table). `EnvProvenance` (adopts devcontainer dichotomy: baked/runtime-applied/probed; carries provenance tags from the 14-source enum in §II). `CredentialSource` (10 classes listed in §II including `macos_keychain`, `long_lived_setup_token`, `api_key_helper`, `1password`, `infisical`, `devenv_secretspec`). `StartupPhase` (14-phase timeline from `boot` → `tool_call_subprocess` per §II.StartupPhase; enables temporal reasoning about env availability). Proposed ADRs 0016 and 0017 now record the ownership boundary and Codex app execution-context split; concrete schema reconciliation with ADR 0015 remains future Ring 0 work.
- **Trap scaffold expansion for #19–#25 and #36** — shell research traps #26–#30 now have scaffold files under `packages/evals/regression/`, matching the #16/#18/#37 scaffold format. Remaining unexpanded scaffolds are #19–#25 from the Cloudflare brief plus #36 `cloudflare-access-token-valid-but-tunnel-audtag-mismatch`; those still require live-provider/tunnel fixtures available in the Phase 1 test harness.
...
| Synthesis | 05-07 -> 05-08 | - | 6 | ADR 0016 + 0017 + 0018 drafts, regression trap scaffolds #26-#30 landed, updated Ring-0 schemas remain next |
