...
4. Scanner parity landed for traps #16 (`ignored-but-load-bearing-deletion`), #17 (`harness-config-boolean-type`), and #18 (`agent-echoes-secret-in-env-inspection`) in `measure-traps.sh`; same-day post-closeout follow-up added advisory scanner heuristics for #37 (`process-argv-secret-exposure`) and #38 (`cloudflare-mcp-mutation-without-fanout-check`). Seed is at 38 (18 prior + 7 Cloudflare Stage 3a + 5 shell/environment research v2 + 5 coordination-lessons brief + 1 Cloudflare tunnel-audience lesson + 1 process-argv secret-exposure lesson + 1 Cloudflare MCP fan-out diagnostics lesson). Traps #19–#36 are seed-only at closeout. Expanded scaffold files remain Phase 1+ work (live-provider/surface fixtures for #19–#30 and #36; coordination/knowledge substrate gated by Q-003 for #31–#35; typed process-inspection operation for #37; broker/fan-out fixture for #38). Hook literal-forbidden-list extension for trap #18's secret-echo regexes landed with this flow.
...
P13 partial evidence: `docs/host-capability-substrate/research/shell-env/2026-04-26-P13-codex-app-bundle-signing.md` captures read-only Codex app bundle/signing metadata plus live process sandbox flags. Current local app evidence is Codex app `26.422.71525` build `2210`; the older closeout baseline was `26.422.30944` build `2080`. Helpers show Electron/Chromium sandbox markers (`--seatbelt-client`, `--enable-sandbox`, `--service-sandbox-type=network`), while entitlement extraction remains unusable and app-internal Keychain/filesystem/network probes remain open. Generated app-server protocol schemas identify typed filesystem/network/account/MCP surfaces; the approved stdio app-server probe initialized successfully and returned `exitCode: 0` for a `/usr/bin/true` `command/exec` status probe, but that temporary server is not GUI app-internal evidence. A 2026-04-28/29 status probe correlated to the active Codex CLI session, not the GUI app. The current blocker is a reachable GUI app-server control path or a human-run sterile Codex app UI probe.
...
Direct-test queue (combined from report 1 §14 + report 2 verification + shell research v2.5.0 P01–P13; blocks work that depends on each outcome):
...
6. Claude Code #18692 (resolved-secrets-into-`.mcp.json`) does NOT repro on 2.1.120+. Local CLI is 2.1.122; host-context Claude auth is available even though sandboxed `claude auth status` can report `loggedIn=false`, so run this from host context if selected.
7. `shell_environment_policy.include_only` reliably exposes named var on Codex CLI 0.125.0 and current Codex macOS app 26.422.71525 (2210). *(Shell research v2 **P04**: schema documented but cross-surface behavior undocumented; issue #3064 suggests divergence. Matrix test with env vectors, 10h.)*
...
14. **NEW — P09**: direnv + mise cross-surface visibility. Initial non-mutating baseline fixture landed as `scripts/dev/run-direnv-mise-fixture.sh` and `just direnv-mise-fixture`: synthetic repo with `.envrc` `HCS_DIRENV_MARKER` + `.mise.toml [env] HCS_MISE_MARKER`, sanitized temp `HOME`/`DIRENV_CONFIG`/`MISE_*` state, no `direnv allow`, no `mise trust`, no GUI launch. The full terminal-launched vs GUI-launched matrix remains open and requires operation proofs for any allow/trust or GUI state.
...
### Phase 1 shell/environment research program (shell research v2.5.0, 2026-04-27 → 2026-05-08)
...
| Wave 2 — genuinely open | 04-30 → 05-06 | P04, P03, P09; P08 surface expansion as available | 30 | Cross-surface matrix + MCP/setup-script trace + direnv/mise matrix; initial Codex CLI provenance snapshot and P09 non-mutating baseline landed |
| Wave 3 — design + prototype (parallel with Wave 2) | 04-29 → 05-06 | P11; P12 Ring 1 design later | 16–20 | LaunchAgent-env policy table; repo-local `hcs env-inspect` prototype landed |
...
- **ADR 0017 — Codex app as distinct ExecutionContext.** Incorporates P13 sandbox characterization. Models Codex app as a separate sub-class of `ExecutionContext` with its own measured capability matrix once app-internal proof exists. Current evidence supports launchd-user-session env inheritance and distinct app/helper sandbox treatment, but Keychain/filesystem/network entries must remain pending until the GUI app supplies status-code evidence. Blocks the "Codex is Codex" mental model; makes the app/CLI divergence first-class.
