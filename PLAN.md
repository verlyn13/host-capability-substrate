...
- **Charter v1.3.0 wave-2 follow-up (post-2026-05-02 review)** — invariants 16 and 17 landed in v1.3.0 wave-1 as charter text only; boundary-enforcement bullets and forbidden-pattern entries that operationalize them were deferred per ADR 0021's charter-and-bookkeeping-only scope. The post-merge `hcs-architect` review flagged this as the first invariant wave in HCS history without enforcement plumbing and recommended a wave-2 PR. Candidate bullets: (a) inv. 17 enforcement — every `OperationShape` carries a resolved `ExecutionContext` reference; missing reference fails policy-lint; (b) inv. 16 enforcement — operations against `provider_kind` capabilities require typed evidence (e.g., `BoundaryObservation` with the matching dimension) before gateway acceptance; (c) forbidden-pattern entries for "rate-limit response treated as retry pressure" and "credential material conflated with `SecretReference`". Wave-2 must remain charter-and-bookkeeping-only (no schema, kernel, or adapter work in the same PR).
