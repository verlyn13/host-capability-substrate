...
4. Scanner parity: add heuristics for traps #16 (`ignored-but-load-bearing-deletion`), #17 (`harness-config-boolean-type`), and #18 (`agent-echoes-secret-in-env-inspection`) to `measure-traps.sh`; seed is at 25 (18 prior + 7 from the Cloudflare Stage 3a brief), scanner catches up to #16/#17/#18 only at closeout. Traps #19–#25 are seed-only at closeout; scanner heuristics + expanded scaffold files are Phase 1 work because they require live-provider fixtures to validate. Hook literal-forbidden-list extension for trap #18's secret-echo regexes lands with this commit so day-over-day `forbidden` classification is consistent before/after closeout.
