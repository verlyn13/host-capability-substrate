...
4. Scanner parity: add heuristics for traps #16 (`ignored-but-load-bearing-deletion`), #17 (`harness-config-boolean-type`), and #18 (`agent-echoes-secret-in-env-inspection`) to `measure-traps.sh`; seed is at 35 (18 prior + 7 Cloudflare Stage 3a + 5 shell/environment research v2 + 5 coordination-lessons brief), scanner catches up to #16/#17/#18 only at closeout. Traps #19–#35 are seed-only at closeout; scanner heuristics + expanded scaffold files are Phase 1+ work (live-provider/surface fixtures for #19–#30; coordination/knowledge substrate gated by Q-003 for #31–#35). Hook literal-forbidden-list extension for trap #18's secret-echo regexes lands with this commit so day-over-day `forbidden` classification is consistent before/after closeout.
