...
- **ADR 0027 Q-006 stage-1 expansion proposed (2026-05-02)** — `docs/host-capability-substrate/adr/0027-q-006-stage-1-source-control-evidence-subtypes.md` proposes three foundational receipts: `GitRepositoryObservation` and `GitRemoteObservation` as `evidenceSchema`-direct typed payloads (Q-011 bucket 1; no new envelopes), and `BranchProtectionObservation` as a `BoundaryObservation` payload for a new `branch_protection` dimension. `GitRemoteObservation` adopts per-(repository, remote_name, ref) grain so "remote-gone" becomes a typed observation rather than an agent assertion (closes the ScopeCam motivating failure at the receipt-shape layer). `GitRepositoryObservation.repository_id` is resolved by a Ring 1 service from `WorkspaceContext`; agent-supplied `repository_id` rejected at the observation-mint API. Multi-remote repositories (fork + upstream + mirror) are first-class via `remote_observation_evidence_refs`. ADR 0027 is doc-only and posture-only (no schema source, no `branch_protection` dimension registration in this commit; that lands with the schema-implementation PR). ADR 0027 awaits human acceptance and reviewer-subagent re-pass; once accepted, unblocks ADR 0025 v2 schema implementation step and ADR 0026 substrate hook architecture. Stage-2 (six remaining Q-006 receipts) queued as a follow-on ADR.
