...
P06 open/narrowed evidence: `docs/host-capability-substrate/research/shell-env/2026-04-26-P06-shell-wrapper-logger-prep.md` captures the in-repo redaction-safe wrapper `scripts/dev/hcs-shell-logger.sh`, fixture `scripts/dev/run-shell-logger-fixture.sh`, approved host install to `/usr/local/bin/hcs-shell-logger`, shebang/atomic-append fixes required by the live run, and clean PATH-routed shell records. The fixture proves the wrapper preserves argv for the real shell while redacting `-c` command payloads from the wrapper log. The approved Codex CLI probe displayed `/bin/zsh -lc` and bypassed PATH routing. A 2026-04-27 Claude-run follow-up captured Claude Code CLI Bash-tool runtime shape via in-tool self-introspection (`/bin/zsh -c` with `login=true`) and reproduced Codex CLI `/bin/zsh -lc` from a non-Codex controlling session (`codex exec --ephemeral --full-auto --json` returned exit 0). Both surfaces use absolute `/bin/zsh` and bypass PATH interception, with different flag forms (`-c` vs. `-lc`). PATH-prefix interception is closed as unsuitable except for negative controls. The next proof is specified in `docs/host-capability-substrate/research/shell-env/2026-04-27-P06-provenance-experiment-plan.md`: tool-native trace, startup-file sentinels, and host-level process telemetry. Avoid nested Codex CLI probes from inside the active Codex session unless cleanup cannot affect the controller.

Direct-test queue (combined from report 1 §14 + report 2 verification + shell research v2.1.0 P01–P13; blocks work that depends on each outcome):
...
11. **NEW — P06**: Shell wrapper-log validation and provenance closure. In-repo wrapper, redaction fixture, and `/usr/local/bin/hcs-shell-logger` host install exist. Approved PATH-routed probes confirmed the wrapper can capture `bash -lc`, `sh -c`, and `zsh -lc` safely after the shebang/atomic-append fixes. Cross-surface runtime shape captured 2026-04-27: Codex CLI = `/bin/zsh -lc` (reproduced from a non-Codex controlling session), Claude Code CLI Bash tool = `/bin/zsh -c` with `login=true` (in-tool self-introspection). Both bypass PATH routing via absolute path. P06 remains open/narrowed; execute `2026-04-27-P06-provenance-experiment-plan.md` to close via tool-native trace, startup-file sentinels, and host-level process telemetry for parent `execve` argv, startup effects, and provenance. 4h remaining.
12. **NEW — P13**: Codex app sandbox boundary characterization (new `ExecutionContext` class). Static bundle/signing plus live helper process flags are captured; generated schema and stdio app-server status probe are complete; app-internal Keychain/filesystem/network status-code probes remain open. 4h.
...
### Phase 1 shell/environment research program (shell research v2.1.0, 2026-04-27 → 2026-05-08)
