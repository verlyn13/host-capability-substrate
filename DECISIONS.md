...
| Q-002 | Are our `[profiles.hcs-plan]`, `[profiles.hcs-implement]`, `[profiles.hcs-review]` in `~/.codex/config.toml` consulted by the Codex macOS app and IDE extension surfaces, or CLI-only? External research report 2 cites OpenAI's primary docs that Codex profiles are experimental and not supported in the IDE extension. Direct test required: invoke Codex via each surface (CLI `codex --profile hcs-plan`; Codex app opened from Dock; IDE extension in VS Code) and observe whether the profile's model/approval/sandbox settings actually apply. Resolution feeds **D-031 (post-closeout)**: formalize Codex profiles as CLI-only opt-ins; HCS does not build cross-surface identity or auth policy around them. | user | first post-closeout direct-test cycle | Pending — profiles confirmed present and in active CLI use 2026-04-23; cross-surface reach unverified. |
